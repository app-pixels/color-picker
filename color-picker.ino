/*
 * color-picker.ino — standalone HSV colour picker
...
void setup() {
  USBSerial.begin(115200); pinMode(0,INPUT_PULLUP);
  Wire.begin(IIC_SDA,IIC_SCL); Wire.setClock(400000);
  if(!power.begin(Wire,AXP2101_SLAVE_ADDRESS,IIC_SDA,IIC_SCL)) USBSerial.println("AXP2101 not found");
  power.disableIRQ(XPOWERS_AXP2101_ALL_IRQ); power.clearIrqStatus();
  power.enableIRQ(XPOWERS_AXP2101_PKEY_SHORT_IRQ); common_init();
  ...
  gfx->setBrightness(g_config.brightness); app_colorpicker_setup(gfx);
}
void loop(){app_colorpicker_loop();}
